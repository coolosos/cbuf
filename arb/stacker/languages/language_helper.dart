...
mixin StrackerHelper on LanguageStracker {
  String formatKeyInfo(
    String hardCodeText, {
    String separator = '',
    required String fileName,
  }) {
    String diacritics = _removeDiacritics(hardCodeText);

    List<String> words = diacritics
        .replaceAll("\"", "")
        .replaceAll(".", "")
        .replaceAll(",", "")
        .replaceAll("?", "")
    ...
    if (fileData.isNotEmpty) {
      fileData[0] = fileData[0].toLowerCase();
    }

    return fileData.join(separator) + '_' + words.join(separator);
  }
...
String _removeDiacritics(String hardCodeText) {
  ...
  if (diacriticsMap.isEmpty) {
    for (int i = 0; i < defaultDiacriticsRemovalap.length; i++) {
      ...
      for (int j = 0; j < (letters?.length ?? 0); j++) {
        diacriticsMap[letters![j]] = defaultDiacriticsRemovalap[i]['base'];
      }
    }
  }

  return hardCodeText.replaceAllMapped(diacriticsRegExp, (a) {
